...
def is_done(sand: tuple[int, int], rocks: set[tuple[int, int]]) -> bool:
    return sand == SAND_START and is_blocked(sand, rocks)
...
def compute(s: str) -> int:
    ...
    max_y = max(r[1] for r in rocks) + 2

    rocks.update((x, max_y) for x in range(
        min(r[0] for r in rocks) - 10000, max(r[0] for r in rocks) + 20000,
    ))

    while not is_done(current_sand, rocks):
