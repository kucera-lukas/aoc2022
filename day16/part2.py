...
def compute(s: str) -> int:
    ...
    res_map: dict[frozenset[str], int] = {}
    search: list[tuple[int, int, str, frozenset[str]]] = [
        (0, 0, 'AA', frozenset()),
    ]
    ...
    while search:
        ...
        res_map[possible] = max(res_map.get(possible, score), score)

        for p in positive_names - possible:
    ...
    for first, second in combinations(res_map.items(), r=2):
        if not first[0] & second[0]:
            max_res = max(max_res, first[1] + second[1])
