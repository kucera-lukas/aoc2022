...
UPPER_BOUND = int(1e99)
SECOND = 21608329599731
...
def compute(s: str, *, stable: int = SECOND) -> int:
    ...
    monkeys['root']['op'] = operator.eq

    def work(monkey: str, monkeys: dict[str, Monkey]) -> int:
        yelled: int | None = monkeys[monkey]['yelled']

        if yelled is None:
            first = work(monkeys[monkey]['first'], monkeys)
            second = work(monkeys[monkey]['second'], monkeys)
    ...
    def search(yell: int) -> int:
        temp_monkeys = deepcopy(monkeys)
        temp_monkeys['humn']['yelled'] = yell

        return work(
            temp_monkeys['root']['first'],
            temp_monkeys,
        )

    direction_flag = search(1) > 0
...
def test(input_s: str, expected: int) -> None:
    assert compute(input_s, stable=150) == expected
